    ...
    public static void start(int port) throws Exception {
        ...
        HttpHandler handler = exchange -> {
            ...
            try {
                if (path.equals("/") && method.equals("GET")) {
                    sendRaw(exchange, 200,
                            "{\"name\":\"Рагу в Гарцующей Кобыле\",\"version\":\"1.0\","
                                    + "\"endpoints\":[\"GET /menu\",\"GET /orders\",\"POST /orders\","
                ...
                sendRaw(exchange, 404,
                        "{\"code\":\"NOT_FOUND\",\"message\":\"Маршрут не найден: " + path + "\"}");

            } catch (Exception e) {
                System.err.println("Ошибка: " + e.getMessage());
                try {
                    sendRaw(exchange, 400,
        ...
        server.createContext("/menu", handler);
        server.createContext("/orders", handler);
        server.createContext("/", handler);
        server.setExecutor(Executors.newFixedThreadPool(4));
        server.start();
        System.out.println("REST API запущен: http://localhost:" + port);
        System.out.println("Меню:    http://localhost:" + port + "/menu");
        System.out.println("Заказы:  http://localhost:" + port + "/orders");
    }

    private static void sendRaw(HttpExchange ex, int status, String json) throws IOException {
